...
# This provided input file contains the fields: player, team, WhoScored rating, minutes played, season for all Premier League players between seasons 09/10 and 18/19
input_file_path = (base_path / "input/football-player-data.csv").resolve()
alldata_unfiltered = genfromtxt(input_file_path, delimiter=',', encoding='utf-8', dtype=str)
cols = alldata_unfiltered[0].size # column names: (0 = player, 1 = team, 2 = rating, 3 = minutes, 4 = season)
...
# detect all seasons
# detect teams in the PL of each season
...
for row_num in range(rows):
        if(int(alldata_unfiltered[row_num][3]) >= minutes_threshold):
            alldata.append([alldata_unfiltered[row_num][0],alldata_unfiltered[row_num][1],alldata_unfiltered[row_num][2],alldata_unfiltered[row_num][3],alldata_unfiltered[row_num][4]])
        else:
            excluded_players.append([alldata_unfiltered[row_num][0],alldata_unfiltered[row_num][1],alldata_unfiltered[row_num][2],alldata_unfiltered[row_num][3],alldata_unfiltered[row_num][4]])
...
seasons_teams_temp = []
for row_num in range(rows):
    seasons_teams_temp.append([alldata[row_num][4],alldata[row_num][1],0,0,0,0,0,0,0])
    # 0s correspond to total players, total ratings, average ratings (which are calculated later on), actual points in that season, actual position in that season, ratings position in that season, total mins played by players in that season
...
player_data_temp = []
missing_players = [] # players who have no talent data to go by (e.g. player wasn't in the league last season)
for row_num in range(len(seasons_teams)):
    for row_num_new in range(rows):
        if(seasons_teams[row_num][0] != "09-10"):
            if(alldata[row_num_new][1] == seasons_teams[row_num][1] and alldata[row_num_new][4] == seasons_teams[row_num][0]):
                ...
                # find player's rating from previous season
                    for x in range(rows):
                        if(alldata[x][0] == player and alldata[x][4] == prev_season):
                            found+=1
                            if(found==1):
                                prev_rating = float(alldata[x][2])
                    ...
                    for x in range(rows):
                        if (alldata[x][0] == player and alldata[x][4] == prev_season):
                            ...
                            if (found == 1):
                                prev_rating1 = float(alldata[x][2])
                            ...
                            if (found > 1):
                                prev_rating2 = float(alldata[x][2])
                        ...
                        if (alldata[x][0] == player and alldata[x][4] == season and alldata[x][1] == team):
...
# calculate the collective WhoScored ratings of all the elites of each team to use as a tiebreaker
...
for x in range(20,len(seasons_teams)): # start from season 10/11, ignoring 09/10 since no talent ratings were calculated for then
    ...
    for b in range(len(excluded_players)):
        if(season == excluded_players[b][4] and team == excluded_players[b][1]):
            excluded_count += 1

    team_elite_talents.append([season, team, elite_count, non_elite_count, missing_count, excluded_count])

# this provided input file contains the fields: season, team, position, points for all Premier League teams between seasons 10/11 and 18/19
input_file_path = (base_path / "input/football-table-data.csv").resolve()
pltables = genfromtxt(input_file_path, delimiter=',', encoding='utf-8', dtype=str)
plcols = pltables[0].size # column names: (0 = season, 1 = team, 2 = position, 3 = points)
pltables = np.delete(pltables,(0),axis = 0) # remove the first row (column names)
plrows = int(pltables.size / plcols) # each row corresponds to a team's position and points in a season

# add number of elite talents as a field to new array
pltables_with_elites = []
for x in range(plrows):
    season = pltables[x][0]
    team = pltables[x][1]
    league_pos = pltables[x][2]
    points = pltables[x][3]
    ...
    for y in range(len(team_elite_talents)):
        if(season == team_elite_talents[y][0] and team == team_elite_talents[y][1]):
            ...
            pltables_with_elites.append([season,team,league_pos,points,num_elites,num_nonelites,num_missing,num_excluded,talent_pos,collective_talents_of_elites,collective_talents_of_nonelites])

# adding talent positions (rankings based on how many talented players a team has in a specific season, compared to other teams + other tiebreakers described below)
pltables_with_elites_sorted = sorted(pltables_with_elites,key=lambda x: (x[0],-x[4],-x[9],x[6],-x[10]))
...
rank = 1
for x in range(len(pltables_with_elites)):
    current_elites = pltables_with_elites_sorted[x][4] # number of elites of current team
    current_elites_season = pltables_with_elites_sorted[x][0] # current season, so as to know when to start ranking again from the next season
    current_missing = pltables_with_elites_sorted[x][6]
    current_collective_elite_talent = pltables_with_elites_sorted[x][9]
    current_collective_nonelite_talent = pltables_with_elites_sorted[x][10]

    # special case
    # first team to be ranked
    if(x == 0):
        pltables_with_elites_sorted[x][8] = 1 # first team to be ranked is always 1st

    if(x > 0):
        previous_elites = pltables_with_elites_sorted[x-1][4] # number of elites of previous team
        previous_elites_season = pltables_with_elites_sorted[x-1][0]
        previous_rank = pltables_with_elites_sorted[x-1][8]
        previous_missing = pltables_with_elites_sorted[x-1][6]
        previous_collective_elite_talent = pltables_with_elites_sorted[x-1][9]
        previous_collective_nonelite_talent = pltables_with_elites_sorted[x-1][10]

    if(x < len(pltables_with_elites)-1):
        next_elites_season = pltables_with_elites_sorted[x+1][0]

    # switching seasons
    if(x > 0 and current_elites_season != previous_elites_season):
        rank = 1
        pltables_with_elites_sorted[x][8] = rank
    ...
    if (x > 0 and current_elites_season == previous_elites_season):
        ...
        if (current_elites == previous_elites):
            ...
            if (current_collective_elite_talent < previous_collective_elite_talent):
                ...
                pltables_with_elites_sorted[x][8] = rank
            if (current_collective_elite_talent == previous_collective_elite_talent):
                # if many teams have the same number collective elite talent, precedence is given to the team which has less missing players
                if(current_missing > previous_missing):
                    rank += 1
                    pltables_with_elites_sorted[x][8] = rank
                if(current_missing == previous_missing):
                    # if many teams have the same number of missing, precedence is given to the team which has higher collective non-elite talent
                    if(current_collective_nonelite_talent < previous_collective_nonelite_talent):
                        rank += 1
                        pltables_with_elites_sorted[x][8] = rank
                    if(current_collective_nonelite_talent == previous_collective_nonelite_talent):
                        pltables_with_elites_sorted[x][8] = previous_rank
                        rank += 1

# output array to file (extended data)
output_file_path = (base_path / "output/football-extended-results.csv").resolve()

with open(output_file_path, 'w') as f:
    print("season,team,league_position,points,elites,non-elites,missing,excluded,talent_position,elite_collective_talent,non-elite_collective_talent",file=f)
    for x in range(len(pltables_with_elites)):
        print("\'" + pltables_with_elites_sorted[x][0] + "\'", pltables_with_elites_sorted[x][1], str(pltables_with_elites_sorted[x][2]), str(pltables_with_elites_sorted[x][3]),
              str(pltables_with_elites_sorted[x][4]), str(pltables_with_elites_sorted[x][5]), str(pltables_with_elites_sorted[x][6]),
              str(pltables_with_elites_sorted[x][7]), str(pltables_with_elites_sorted[x][8]), str(pltables_with_elites_sorted[x][9]),
              str(pltables_with_elites_sorted[x][10]), sep=",",file=f)

# output array to file (truncated data)
output_file_path = (base_path / "output/football-truncated-results.csv").resolve()

with open(output_file_path, 'w') as f:
    print("season,team,elites,league_position,talent_position",file=f)
    for x in range(len(pltables_with_elites)):
        print("\'" + pltables_with_elites_sorted[x][0] + "\'", pltables_with_elites_sorted[x][1], str(pltables_with_elites_sorted[x][4]),
              str(pltables_with_elites_sorted[x][2]), str(pltables_with_elites_sorted[x][8]),sep=",",file=f)
...
for x in range(len(pltables_with_elites)):
    talent_positions = np.append(int(pltables_with_elites_sorted[x][8]),talent_positions)
    league_positions = np.append(int(pltables_with_elites_sorted[x][2]),league_positions)

print("Pearson Correlation Coefficient between Talent Position and League Position [FOOTBALL]: ")
